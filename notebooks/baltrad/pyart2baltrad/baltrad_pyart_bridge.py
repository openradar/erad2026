...
def _fillscan(scan, radar, index=0):
    ...
    if radar.instrument_parameters is not None:
        ...
        if "nyquist_velocity" in inst_params:
            scan.addAttribute(
                "how/NI", float(inst_params["nyquist_velocity"]["data"][startray])
            )
...
def raveio2radar(rio, raw=False):
    ...
    if rio.objectType is _rave.Rave_ObjectType_SCAN:
        ...
        raise TypeError("Unsupported object, only SCANs and PVOLs supported.")
    ...
    sweep_start_ray_index["data"] = np.cumsum(
        np.append([0], rays_per_sweep[:-1])
    ).astype("int32")
    sweep_end_ray_index["data"] = np.cumsum(rays_per_sweep).astype("int32") - 1
    ...
    _range["data"] = np.arange(max_bins, dtype="float32") * rscales[0] + rstarts[0]
    ...
    if rio.objectType is _rave.Rave_ObjectType_SCAN:
        if "how/startazA" in first_scan.getAttributeNames():
            ...
            sweep_az = np.where(np.greater(sweep_az, 360.0), sweep_az - 360.0, sweep_az)
            az_data[start : start + first_scan.nrays] = sweep_az
        else:
            az_data = np.arange(first_scan.nrays) + (360.0 / first_scan.nrays / 2)
        ...
        for s in range(nsweeps):
            ...
            if "how/startazA" in scan.getAttributeNames():
                ...
                sweep_az = np.where(
                    np.greater(sweep_az, 360.0), sweep_az - 360.0, sweep_az
                )
                az_data[start : start + scan.nrays] = sweep_az
                start += scan.nrays
            else:
                az_data[start : start + scan.nrays] = np.arange(scan.nrays) + (
                    360.0 / scan.nrays / 2
                )
    ...
    if "how/startepochs" in attrnames and "how/stopepochs" in attrnames:
        ...
        start_epoch = time.mktime(
            datetime.datetime.strptime(
                first_scan.startdate + first_scan.starttime, "%Y%m%d%H%M%S"
            ).timetuple()
        )
        end_epoch = time.mktime(
            datetime.datetime.strptime(
                first_scan.enddate + first_scan.endtime, "%Y%m%d%H%M%S"
            ).timetuple()
        )
    ...
    beam_width_h["data"] = np.array([first_scan.beamwidth * rd], dtype="float32")
...
def _collect_attrs(rio, attr):
    ...
    if rio.objectType is _rave.Rave_ObjectType_SCAN:
        ...
        collected = [
            getattr(rio.object.getScan(i), attr)
