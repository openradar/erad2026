...
```{code-cell} ipython3
import cmweather  # registers ChaseSpectral and other radar colormaps
import fsspec
import icechunk
...
Use xradar's `.xradar.georeference()` to compute Cartesian coordinates (`x`, `y`, `z`) from the native polar grid, then plot with xarray's built-in `.plot()` method and the `ChaseSpectral` colormap from `cmweather`.

### Side-by-side PPI — FGora vs Jastrebac
...
for ax, (dt, task, site) in zip(axes, [
    ...
    ds["DBZH"].sel(vcp_time="2014-05-15", method="nearest").plot(
        x="x", y="y",
...
```{code-cell} ipython3
ds_j = (
    jastrebac_dt["/JSTB_250_Dp_leto/sweep_0"]
    .to_dataset(inherit="all_coords")
    .xradar.georeference()
)

fig, axes = plt.subplots(1, 3, figsize=(17, 5))
for ax, (mom, cmap, vmin, vmax) in zip(axes, [
    ("DBZH",  "ChaseSpectral", -10,  70),
    ("ZDR",   "RdYlBu_r",      -2,   5),
    ("RHOHV", "viridis",       0.7, 1.0),
]):
    ds_j[mom].sel(vcp_time="2014-05-15", method="nearest").plot(
        x="x", y="y",
